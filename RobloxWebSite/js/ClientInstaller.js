...
// Define the ROBLONIUM URI launcher object functions
        ...
        pluginObj.StartGame = function(authenticationUrl, visitUrl) {
                var uri = "novetus://launch?client=" + encodeURIComponent("2013L") +
                    "&script=" + encodeURIComponent(visitUrl);
                if (pluginObj.AuthenticationTicket) {
                    uri += "&ticket=" + encodeURIComponent(pluginObj.AuthenticationTicket);
                }
                document.getElementById("downloadInstallerIFrame").src = uri;
...
Roblox.Client.WaitForRoblox = function (continuation) {
    ...
    if (Roblox.Client.IsRobloxProxyInstalled()) {
        ...
        */
        ...
        // try to download
        window.open('https://github.com/nicemre/Novetus/releases/latest', '_blank', 'noopener,noreferrer');
    }
